...
const typeOptions = computed(() => props.types.map(tp => ({ value: tp, label: t(`meetings.type.${tp}`) })));
const unitOptions = computed(() => [
    { value: null, label: t('meetings.field.holding_wide') },
    ...props.units.map(u => ({ value: u.id, label: u.name, color: u.color })),
]);
const statusOptions = computed(() => props.statuses.map(s => ({ value: s, label: t(`meetings.status.${s}`) })));
</script>
